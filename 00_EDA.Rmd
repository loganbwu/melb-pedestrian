...
The data contains both daily and weekly seasonality. Some R functions do not handle multiple seasonalities. We can pre-process to remove the known seasonalities.
...
The final differenced data appears stationary. Spikes could be due to public holidays, which will be ignored for now. Intuitively, we also need at least a first differencing (AR1) but this can be done in the ARIMA fit.
